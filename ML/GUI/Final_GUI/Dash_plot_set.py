...
def update_candlestick_chart(value,timeframe):
    ...
    if timeframe == '1H':
        df_plot = df_hour[(df_hour['Symbol'] == value)].tail(150)
        df_plot['Datetime'] = pd.to_datetime(df_plot['Datetime'])
        df_date = df_plot['Datetime']
    elif timeframe == '1D':
        df_plot = df_day[(df_day['Symbol'] == value)].tail(150)
    ...
    candles.add_trace(go.Candlestick(x=df_date,open=df_plot['Open'],
    ...
    candles.add_trace(go.Scatter(x=df_date, y=df_plot['MA25'] , line=dict(color='yellow')), row=1, col=1)
